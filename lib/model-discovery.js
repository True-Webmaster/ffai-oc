...
class ModelDiscovery {
   ...
   * @param {number}  [opts.refreshIntervalMs] - Periodic refresh interval (default: 300000 / 5min)
   */
  constructor({ pool, logger, refreshIntervalMs = 300000, minTpm = DEFAULT_MIN_TPM,
  ...
  /**
   * Get cached discovered models for one provider.
  ...
  startPeriodicRefresh() {
    ...
    this._timer = setInterval(() => {
      this.refresh().catch(err => {
        this._logger.warn(`[discovery] periodic refresh failed: ${err.message}`);
      });
