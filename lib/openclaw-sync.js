 ...
 * @param {string[]} [favorites] - Favorite model IDs to mirror into plugins.entries.ffai.config.favorites
 * @returns {boolean} - true if written successfully
 */
function writeOpenclawJson(openclawJsonPath, ocProviders, logger = console, favorites = null) {
  let oc;
  try {
  ...
  // Mirror FFAI favorites into plugins.entries.ffai.config.favorites so that
  // OpenClaw plugin-driven views (e.g. the Telegram /models command) stay in
  // sync with FFAI's source-of-truth config.favorites. Passing null leaves
  // it untouched (used by the bare sync-openclaw.js CLI path).
  if (Array.isArray(favorites)) {
    if (!oc.plugins) oc.plugins = {};
    if (!oc.plugins.entries) oc.plugins.entries = {};
    if (!oc.plugins.entries.ffai) oc.plugins.entries.ffai = { enabled: true, config: {} };
    if (!oc.plugins.entries.ffai.config) oc.plugins.entries.ffai.config = {};
    oc.plugins.entries.ffai.config.favorites = favorites.slice();
  }
...
function sync(opts) {
  ...
  if (!writeOpenclawJson(openclawJson, ocProviders, logger, favorites)) return false;
...
module.exports = { buildProviderEntries, buildFavoritesProvider, writeOpenclawJson, writeAgentModels, sync, PROVIDER_PREFIX };
