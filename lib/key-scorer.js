...
class KeyScorer {
  ...
  /** Cancel a pending acquire without recording a request (for error paths that don't reach release). */
  cancelPending(key) {
    this._decrementPending(key);
  }
  ...
  /**
   * Reset all learned state for a key (useful for key rotation or testing).
   * @param {string} key
   */
  reset(key) {
    this.learnedRpm.delete(key);
    this.learnedRpmTs.delete(key);
    this.learnedTpm.delete(key);
    this.learnedTpmTs.delete(key);
    this.learnedRpd.delete(key);
    this.learnedRpdTs.delete(key);
    this.consecutiveErrors.set(key, 0);
    this.recentErrors.set(key, 0);
    this.recentRequests.set(key, 0);
    this.keyCbUntil.delete(key);
    this.keyCbBackoff.set(key, 1);
    this.pending.set(key, 0);
    this.activeStreams.set(key, 0);
    const w = this.windows.get(key);
    if (w) w.reset();
    const mw = this.modelWindows.get(key);
    if (mw) mw.clear();
    const daily = this.dailyUsage.get(key);
    if (daily) { daily.requests = 0; daily.tokens = 0; }
  }
